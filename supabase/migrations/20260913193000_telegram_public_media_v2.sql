...
set thumbnail_url = metadata->>'media_preview_url',
    media_url = case when content_type::text='IMAGE' and media_url is null then metadata->>'media_preview_url' else media_url end,
    updated_at = now()
where source_type='TELEGRAM'
  and thumbnail_url is null
  and coalesce(metadata->>'media_preview_url','') ~ '^https?://';
...
as $$
declare rec record; queued integer := 0;
begin
  for rec in
    select s.id

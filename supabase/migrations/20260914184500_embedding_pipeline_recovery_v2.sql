create or replace function public.claim_embedding_jobs(p_batch integer default 20)
  ...
  with claimed as (
    ...
    where j.content_id in (
      ...
      where (q.status='PENDING' or (q.status='FAILED' and q.attempts < 5))
        and nullif(btrim(coalesce(c.text_content,'')), '') is not null
      order by
        case when coalesce(c.published_at,c.created_at) >= now()-interval '14 days' then 0 else 1 end,
        coalesce(f.normalized_quality,c.quality_score,0.5) desc,
        coalesce(c.published_at,c.created_at) desc,
        q.requested_at asc
      limit greatest(least(p_batch,100),1)

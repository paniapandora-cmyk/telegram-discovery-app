-- Record every Telegram join/leave for registered creator channels.
-- Referral attribution remains separate and is used only for conversion.
...
create index if not exists creator_events_member_state_idx
...
as $function$
  select
    ...
    coalesce(mm.telegram_joins, 0),
    coalesce(am.telegram_active_joins, 0),
    coalesce(mm.telegram_leaves, 0),
    case
  ...
  left join lateral (
    ...
    where ce.creator_id = cr.id
      and ce.event_type in ('join', 'leave')
  ) mm on true
  left join lateral (
    select count(*) as telegram_active_joins
    from (
      select distinct on (
        ce.creator_channel_id,
        ce.telegram_user_id
      )
        ce.event_type
      from public.creator_events ce
      where ce.creator_id = cr.id
        and ce.telegram_user_id is not null
        and ce.event_type in ('join', 'leave')
      order by
        ce.creator_channel_id,
        ce.telegram_user_id,
...
as $function$
  ...
  select
    ...
    coalesce(mm.telegram_joins, 0)::bigint,
    coalesce(am.active_joins, 0)::bigint,
    coalesce(mm.leaves, 0)::bigint,
    ...
    case
      ...
      else coalesce(ref.attributed_joins, 0)::double precision
  ...
  left join lateral (
    select
      count(*) filter (
        where m.event_type = 'join'
      ) as telegram_joins,
      count(*) filter (
        where m.event_type = 'leave'
      ) as leaves
    from public.creator_events m
    where m.creator_channel_id = ch.id
      and m.event_type in ('join', 'leave')

...
async function rest(path: string, init: RequestInit = {}) {
  if (!SUPABASE_URL || !SERVICE_KEY) throw new Error("related_backend_not_configured");
  ...
  if (!response.ok) {
    throw new Error(body?.message || body?.hint || body?.details || `rest_${response.status}`);
  }
...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (!UUID.test(contentId)) {
      return json({ ok: false, error: "valid_content_id_required", items: [] }, 400);
    }

    const userId = await ensureUser(auth.user);
    const rows = await rest("rpc/get_related_content_v1", {
      method: "POST",
      body: JSON.stringify({
        p_user_id: userId,
    ...
    return json({
      ...
      count: items.length,
      algorithm: "semantic-topic-personalized-v1",
      items,
    });
  } catch (error) {
    console.error("related-content-v1", error);
    return json({
      ok: false,

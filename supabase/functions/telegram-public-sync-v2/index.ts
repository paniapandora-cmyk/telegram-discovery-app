...
async function fetchPage(username:string,before=0):Promise<Page>{
  ...
  const r=await fetch(url,{headers:{"user-agent":"Mozilla/5.0 (compatible; TelegramDiscovery/2.2; +https://t.me)","accept-language":"fa,en;q=0.8"},redirect:"follow"});
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const creatorUpdate=await db.from("creators").update({name:latest.channelTitle||source.title||username,bio:latest.channelBio||undefined,avatar_url:latest.avatar||undefined,source_url:`https://t.me/${username}`,updated_at:new Date().toISOString()}).eq("id",creatorId);
    if(creatorUpdate.error)console.error("creator metadata refresh failed",creatorUpdate.error.message);
    ...
    for(const p of posts){
      try{
        ...
        if(itemErrors.length<8)itemErrors.push({message_id:p.id,error:message.slice(0,500)});
        console.error("public sync item failed",p.id,message);
      }
    }
    ...
    if(db&&runId)await db.from("telegram_sync_runs").update({status:"failed",completed_at:new Date().toISOString(),error_message:message}).eq("id",runId).then(()=>{},()=>{});

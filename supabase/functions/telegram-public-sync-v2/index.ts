...
const errorText=(error:unknown)=>{if(error instanceof Error)return error.message;if(error&&typeof error==='object'){try{return JSON.stringify(error);}catch{return String(error);}}return String(error??'unknown error');};
const sanitizeText=(v:unknown)=>{
  let s=String(v??"");
  const wellFormed=(s as string & {toWellFormed?:()=>string}).toWellFormed;
  if(typeof wellFormed==="function")s=wellFormed.call(s);
  return s.replace(/\u00a0/g," ").replace(/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/g,"").replace(/\r\n?/g,"\n").split("\n").map(x=>x.trimEnd()).join("\n").trim();
};
const publishedAt=(node:any)=>{
  const candidates=[
    node.querySelector("time[datetime]")?.getAttribute?.("datetime"),
    node.querySelector(".tgme_widget_message_date time")?.getAttribute?.("datetime"),
    node.querySelector(".tgme_widget_message_date")?.getAttribute?.("datetime"),
...
type ItemError={message_id:number;error:string};
...
async function fetchPage(username:string,before=0):Promise<Page>{
  ...
  const r=await fetch(url,{headers:{"user-agent":"Mozilla/5.0 (compatible; TelegramDiscovery/2.3; +https://t.me)","accept-language":"fa,en;q=0.8"},redirect:"follow"});
  if(!r.ok)throw new Error(`Telegram public preview returned HTTP ${r.status}`);
  const root=parse(await r.text());
  const channelTitle=sanitizeText(root.querySelector(".tgme_channel_info_header_title, .tgme_page_title")?.innerText)||null;
  const channelBio=sanitizeText(root.querySelector(".tgme_channel_info_description, .tgme_page_description")?.innerText)||null;
  ...
  for(const node of root.querySelectorAll(".tgme_widget_message")){
    ...
    const text=sanitizeText(node.querySelector(".tgme_widget_message_text")?.innerText);
    const mi=media(node);if(!text&&mi.contentType==="TEXT")continue;
    unique.set(id,{id,text,publishedAt:publishedAt(node),...mi});
  }
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const requestClaim=await db.rpc("claim_telegram_sync_request_v1",{p_source_id:sourceId,p_token:requestToken});if(requestClaim.error)throw requestClaim.error;if(!requestClaim.data)return json({ok:false,error:"sync request is invalid, expired, or already used"},403);
    ...
    if(!creatorId){const ins=await db.from("creators").insert({name:sanitizeText(source.title)||username,username,source_url:`https://t.me/${username}`,is_active:true}).select("id").single();if(ins.error)throw ins.error;creatorId=ins.data.id;}
    const run=await db.from("telegram_sync_runs").insert({source_id:sourceId,mode:Number(source.sync_cursor||0)>0?"incremental":"backfill",status:"running",metadata:{sync_provider:"telegram_public_preview",sync_version:SYNC_VERSION,request_auth:"one_time_token"}}).select("id").single();if(!run.error)runId=run.data.id;
    ...
    const creatorUpdate=await db.from("creators").update({name:latest.channelTitle||sanitizeText(source.title)||username,bio:latest.channelBio||undefined,avatar_url:latest.avatar||undefined,source_url:`https://t.me/${username}`,updated_at:new Date().toISOString()}).eq("id",creatorId);if(creatorUpdate.error)console.error("creator metadata refresh failed",creatorUpdate.error.message);
    ...
    let inserted=0,updated=0,errors=0,newest=Number(source.sync_cursor||0),oldest=historyBefore>0?historyBefore:0;const itemErrors:ItemError[]=[];const peerKey=String(source.telegram_peer_id??username);

    for(const p of posts){
      try{
        newest=Math.max(newest,p.id);oldest=oldest>0?Math.min(oldest,p.id):p.id;const sourceKey=`${peerKey}:${p.id}`;
        const ex=await db.from("contents").select("id,metadata,moderation_status,thumbnail_url,media_url,published_at").eq("source_type","TELEGRAM").eq("source_id",sourceKey).limit(1);if(ex.error)throw ex.error;
        const old=ex.data?.[0];const thumb=p.preview||old?.thumbnail_url||old?.metadata?.media_preview_url||null;const safePublishedAt=p.publishedAt||old?.published_at||null;
        const content={creator_id:creatorId,source_type:"TELEGRAM",source_id:sourceKey,source_url:`https://t.me/${username}/${p.id}`,content_type:p.contentType,title:p.text?p.text.slice(0,160):null,description:p.text||null,rights_status:"REFERENCE_ONLY",moderation_status:old?.moderation_status||"PENDING",published_at:safePublishedAt,text_content:p.text||null,thumbnail_url:thumb,media_url:p.contentType==="IMAGE"?(p.preview||old?.media_url||null):(old?.media_url||null),metadata:{...(old?.metadata||{}),telegram_source_id:sourceId,telegram_peer_id:String(source.telegram_peer_id??""),telegram_message_id:p.id,public_preview_sync:true,sync_provider:"telegram_public_preview",sync_version:SYNC_VERSION,media_preview_url:p.preview,deleted_on_telegram:false,published_at_missing:safePublishedAt===null}};
      ...
      }catch(e){errors++;const message=errorText(e);if(itemErrors.length<8)itemErrors.push({message_id:p.id,error:message.slice(0,500)});console.error("public sync item failed",p.id,message);}
    }
    ...
    if(runId)await db.from("telegram_sync_runs").update({status,completed_at:now,fetched_count:posts.length,inserted_count:inserted,updated_count:updated,error_count:errors,last_message_id:newest||null,error_message:errors?`${errors} public-preview item(s) failed`:null,metadata:{sync_provider:"telegram_public_preview",sync_version:SYNC_VERSION,request_auth:"one_time_token",latest_count:latest.posts.length,history_count:(history.posts||[]).length,history_before_id:oldest,history_complete:historyComplete,channel_metadata_enriched:true,item_errors:itemErrors}}).eq("id",runId);
    return json({ok:true,provider:"telegram_public_preview",sync_version:SYNC_VERSION,source_id:sourceId,username,status,fetched:posts.length,inserted,updated,errors,item_errors:itemErrors,sync_cursor:newest,history_before_id:oldest,history_complete:historyComplete,channel_metadata_enriched:true,run_id:runId});
  }catch(e){const message=errorText(e);console.error("telegram-public-sync-v2 fatal",message);if(db&&runId)await db.from("telegram_sync_runs").update({status:"failed",completed_at:new Date().toISOString(),error_message:message}).eq("id",runId).then(()=>{},()=>{});return json({ok:false,error:message},500);}finally{if(db&&leaseSource)await db.from("telegram_sources").update({sync_lease_token:null,sync_lease_until:null}).eq("id",leaseSource).eq("sync_lease_token",leaseToken);}
});

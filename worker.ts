 ...
 *
 * The Worker serves static assets and proxies application APIs
 * to the active Supabase Edge Functions.
 */
...
async function telegramSearch(
  ...
  const query =
    (incoming.searchParams.get("q") || "").trim();
  const limit =
...
export default {
  async fetch(
    ...
    if (
      url.pathname === "/api/telegram/search"
    ) {
      return telegramSearch(request);
    }

    if (
      url.pathname === "/api/telegram/health"
    ) {

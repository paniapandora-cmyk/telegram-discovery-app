...
export default function Viewer({
  ...
  const mediaCandidates = useMemo(() => {
    const localizedDirect = post.mediaUrl?.startsWith(`${ORIGIN}/api/`)
      ? `${previewOrigin}${post.mediaUrl.slice(ORIGIN.length)}`
      : post.mediaUrl;
    ...
    <section className={`viewer viewer-${post.kind}`}>
      ...
      <div
        className={[
          'viewerMedia',
          `tone-${post.tone}`,
          mediaReady ? 'viewerHasMedia' : 'viewerNoMedia',
          activeMedia && !mediaReady && !mediaExhausted
            ? 'viewerMediaLoading'
            : '',
        ]
          .filter(Boolean)
      ...
      >
        ...
        {!mediaReady && activeMedia && !mediaExhausted && (
          <span className="viewerMediaShimmer" aria-hidden="true" />
        )}

        <span className="categoryChip">{post.category}</span>

        {post.kind === 'video' && (
          <span className="playBadge viewerPlay">
            <Play fill="currentColor" />
          </span>
        )}

        {!mediaReady && (
          <span className="textQuoteMark viewerQuote">“</span>
        )}
      ...
      <div className="viewerChannel">
        ...
        <div>
          ...
          <small>{post.channel.username}</small>
        </div>
      ...
      <article className="viewerText">
        ...
        {showFeedback && (
          <div className="viewerFeedback">
            <button onClick={notInterested} disabled={feedbackBusy}>
              <EyeOff />
              {feedbackBusy ? 'در حال ثبت…' : 'به من نشان نده'}
            </button>
            <span>
              این بازخورد برای بهتر شدن پیشنهادهای بعدی استفاده می‌شود.
            </span>
          </div>
        )}
      </article>
      ...
      <button
        className="telegramCta"
        onClick={() => openTelegramPost(post)}
        disabled={!hasTelegram}
      >
        <ExternalLink />
        {hasTelegram
          ? 'دیدن پست در تلگرام'
          : 'لینک تلگرام در دسترس نیست'}
      </button>
    </section>
  );
}

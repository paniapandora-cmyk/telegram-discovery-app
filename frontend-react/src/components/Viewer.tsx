...
export default function Viewer({ post, onClose, onToggleSave, onFeedback, onOpenRelated, onOpenChannel }: Props) {
  ...
  const mediaCandidates = useMemo(() => postMediaCandidates(post), [post.id, post.mediaUrl, post.telegramUrl]);

  const loadRelated = (signal?: AbortSignal) => {
    if (!post.contentId) {
      setRelated([]); setRelatedError(''); setRelatedLoading(false); return Promise.resolve();
    }
    ...
    return loadRelatedPosts(post, signal)
      ...
      .finally(() => { if (!signal?.aborted) setRelatedLoading(false); });
  };
  ...
  const feedback = async (type: string) => {
    ...
    try { await onFeedback(post, type); } finally { setFeedbackBusy(''); }
  };
  const share = async () => {
    if (shareBusy) return;
    setShareBusy(true);
    try { await shareDiscoveryPost(post); } finally { setShareBusy(false); }
  };
  const toggleSave = () => {
    if (post.sponsored && post.promotionId) void recordPromotionEvent(post, 'save').catch(() => {});
      ...
      <section className="relatedSectionV12" aria-label="پست‌های مرتبط">
        ...
        {relatedLoading ? (
          ...
          <div className="relatedRailV12">
            {related.map((item) => (
              <article key={item.id} className="relatedCardV12" onClick={() => onOpenRelated(item)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onOpenRelated(item); } }} role="button" tabIndex={0}>
                <LibraryThumb post={item} className="relatedThumbV12" showKind={false} />
                <div><span className="relatedReasonV12"><Sparkles /> {item.relatedReason || 'مرتبط با این پست'}</span><strong>{item.title}</strong><small>{item.channel.title}</small></div>
              </article>
            ))}
          </div>
        ) : relatedError ? (

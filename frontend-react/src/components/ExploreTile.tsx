...
export default function ExploreTile({
  ...
  const failMedia = () => {
    ...
    if (mediaIndex + 1 < candidates.length) {
      setMediaIndex((value) => value + 1);
    } else {
      setMediaIndex(candidates.length);
    }
  };
  ...
  return (
    <article
      ...
      className={[
        ...
        tall ? 'tall' : '',
        compact ? 'compact' : '',
        mediaReady ? 'hasMedia' : 'noMedia',
        activeMedia && !mediaReady && !exhausted ? 'loadingMedia' : '',
      ]
        .filter(Boolean)
        .join(' ')}
      onClick={() => onOpen(post)}
    ...
    >
      ...
      {!mediaReady && activeMedia && !exhausted && (
        <span className="exploreMediaShimmer" aria-hidden="true" />
      )}

      <div className="exploreTileOverlay" />

      <span className="exploreCategory">{post.category}</span>

      {post.kind === 'video' && (
        <span className="explorePlay">

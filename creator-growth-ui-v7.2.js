/*
 * Telegram Discovery — Creator Growth UI v7.2 — Creator Center Polish
...
(() => {
  ...
  function concise(value) {
    const n = num(value);
    if (!n) return "0";
    if (Math.abs(n) < 1000) return en.format(n);
    try {
  ...
  function installHostTheme() {
    ...
    style.textContent = `
      ...
      .head{margin:2px 2px 8px!important;align-items:center!important;gap:8px!important}
      ...
      .head span{display:block!important;color:#777!important;font-size:7px!important}
      ...
      .native-card{min-height:0!important;background:#181818!important}
      .nativePreview,.document-card .nativePreview,.audio-card .nativePreview{
        ...
        grid-template-rows:auto auto auto!important;align-content:start!important;
      ...
      .hero .nativePreview{height:auto!important;min-height:0!important;padding:10px!important}
      ...
      .nativeMessage,.hero .nativeMessage{
        margin:0!important;padding:4px 0 2px!important;color:#E9E9E9!important;font-size:12px!important;font-weight:450!important;line-height:19px!important;
        display:-webkit-box!important;-webkit-box-orient:vertical!important;-webkit-line-clamp:5!important;line-clamp:5!important;overflow:hidden!important;
      }
      .nativeFoot{
        margin-top:6px!important;gap:6px!important;padding-top:7px!important;
        border-top:1px solid #292929!important;color:#909090!important;font-size:8px!important;
      }
      ...
      .media-card>.save,.native-card>.save{display:none!important}
      ...
      .v6-creator-card{
        flex:0 0 136px;min-height:132px;display:flex;flex-direction:column;align-items:center;padding:9px 8px 8px;
        border:1px solid #2B2B2B;border-radius:14px;background:#1B1B1B;box-shadow:0 3px 10px rgba(0,0,0,.14);text-align:center;
      }
      .v6-creator-card img,.v6-creator-avatar-fallback{
        width:48px;height:48px;flex:0 0 48px;border:1px solid #343434;border-radius:50%;object-fit:cover;background:#232323;
      }
      ...
      .v6-creator-card button{
        width:100%;min-height:28px;margin-top:auto;border:0;border-radius:999px;color:#fff;background:var(--v6-blue);font-size:8px;font-weight:700;cursor:pointer;
      }
  ...
  function decorateMediaCards() {
    ...
    qa("#grid .media-card, #grid .native-card").forEach((card) => {
  ...
  creatorRoot.innerHTML = `
    <style>
      ...
      *{box-sizing:border-box}
      ...
      .wrap{width:min(100%,700px);margin:auto;padding:calc(env(safe-area-inset-top,0px) + 40px) 10px 24px}
      .top{position:sticky;top:0;z-index:3;display:flex;align-items:center;justify-content:space-between;gap:10px;padding:4px 0 12px;background:#0A0A0A}
      .identity{display:flex;align-items:center;gap:10px;min-width:0}
      .logo{width:36px;height:36px;display:grid;place-items:center;border-radius:12px;background:#FF7147;color:#fff;font-weight:900;font-size:14px;flex:0 0 36px}
      .title{min-width:0}
      .title b{display:block;font-size:14px;line-height:18px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
      .title span{display:block;margin-top:2px;color:#858585;font-size:8px;line-height:11px}
      .close{width:32px;height:32px;border:1px solid #303030;border-radius:10px;color:#D8D8D8;background:#171717;cursor:pointer;flex:0 0 32px}
      .toolbar{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:7px;margin-bottom:10px}
      .channels{display:flex;gap:6px;overflow:auto;padding-bottom:2px;scrollbar-width:none}
      .channels::-webkit-scrollbar{display:none}
      .chip{flex:0 0 auto;height:31px;padding:0 10px;border:1px solid #373737;border-radius:999px;color:#BDBDBD;background:#101010;font-size:9px;cursor:pointer;white-space:nowrap}
      ...
      select{height:31px;padding:0 9px;border:1px solid #333;border-radius:10px;color:#D7D7D7;background:#161616;outline:0;font-size:9px}
      .overview{display:flex;align-items:center;justify-content:space-between;gap:10px;margin:0 1px 9px}
      .overview b{font-size:11px;line-height:15px;color:#EDEDED}
      .overview span{color:#777;font-size:7px}
      .channelState{display:grid;grid-template-columns:auto minmax(0,1fr) auto;grid-template-areas:"dot title username" "status status status";align-items:center;gap:3px 8px;margin-bottom:9px;padding:10px;border:1px solid #282828;border-radius:12px;background:#151515}
      .channelState .dot{grid-area:dot;width:8px;height:8px;border-radius:50%;background:#37C98B}
      .channelState .name{grid-area:title;font-size:11px;font-weight:700;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
      .channelState .username{grid-area:username;color:#B7B7B7;font-size:8px;white-space:nowrap}
      .channelState .status{grid-area:status;color:#787878;font-size:7px}
      .metrics{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
      .metric{min-width:0;min-height:64px;padding:10px;border:1px solid #2B2B2B;border-radius:12px;background:#181818;display:flex;flex-direction:column;justify-content:space-between}
      ...
      .metric strong{display:block;margin-top:6px;color:#F5F5F5;font-size:18px;line-height:19px;font-weight:820;white-space:nowrap;font-variant-numeric:tabular-nums;letter-spacing:.1px}
      .metric .hint{display:block;margin-top:6px;color:#707070;font-size:7px;line-height:9px}
      .section{margin-top:10px;padding:10px;border:1px solid #2B2B2B;border-radius:13px;background:#151515}
      ...
      .funnelRow{display:grid;grid-template-columns:68px minmax(0,1fr) 48px;align-items:center;gap:7px;min-height:28px}
      ...
      .track{height:8px;overflow:hidden;border-radius:999px;background:#242424}
      ...
      .item{padding:10px;border:1px solid #292929;border-radius:11px;background:#181818}
      .itemTitle{font-size:10px;font-weight:650;line-height:15px;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden}
      .mini{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:5px;margin-top:8px}
      .mini div{min-width:0;padding:6px 3px;border-radius:8px;background:#101010;text-align:center}
      ...
      .error{color:#FF7D85}
      .foot{display:flex;align-items:center;gap:6px;margin-top:9px;color:#777;font-size:7px}
      .foot button{margin-inline-start:auto;min-height:32px;padding:0 10px;border:0;border-radius:10px;color:#fff;background:#2AABEE;cursor:pointer;font-size:8px;font-weight:700}
      .foot button:disabled{opacity:.45;cursor:default}
      @media(max-width:340px){.wrap{padding-top:calc(env(safe-area-inset-top,0px) + 34px)}.metrics{grid-template-columns:repeat(2,minmax(0,1fr))}.metric{min-height:60px}.metric strong{font-size:16px}.funnelRow{grid-template-columns:58px minmax(0,1fr) 40px}.mini{grid-template-columns:repeat(3,minmax(0,1fr))}}
      @media(min-width:560px){.sheet{inset:4dvh max(10px,calc((100vw - 720px)/2)) auto;border:1px solid #292929;border-radius:20px;max-height:92dvh;box-shadow:0 18px 60px rgba(0,0,0,.48)}.wrap{padding-top:16px}}
  ...
  function renderCreatorBody() {
    ...
    body.innerHTML = `
      ...
      <div class="channelState">
        <span class="dot"></span>
        <b class="name">${escapeHtml(channel?.title || dashboard.title || "کانال")}</b>
        ${channel?.username ? `<span class="username">@${escapeHtml(channel.username)}</span>` : `<span class="username"></span>`}
        <span class="status">${escapeHtml(statusText)}</span>
      </div>

      <div class="metrics">
        ${metric("Views", concise(dashboard.views), "نمایش", false)}
        ${metric("Unique", concise(dashboard.uniqueViewers), "کاربر یکتا", false)}
        ${metric("TG Opens", concise(dashboard.telegramOpens), "بازشدن تلگرام", false)}
        ${metric("Join Clicks", concise(dashboard.joinClicks), "کلیک عضویت", false)}
        ${metric("TG Joins", concise(dashboard.telegramJoins), "عضویت", true)}
        ${metric("Active", concise(dashboard.activeJoins), "فعال", false)}
        ${metric("Leaves", concise(dashboard.leaves), "خروج", false)}
      ...
      <section class="section">
        ...
        <div class="funnel">
          <div class="funnelRow"><span>Views</span><div class="track"><div class="bar" style="width:${funnelWidth(dashboard.views, views)}%"></div></div><b>${concise(dashboard.views)}</b></div>
          <div class="funnelRow"><span>TG Opens</span><div class="track"><div class="bar purple" style="width:${funnelWidth(dashboard.telegramOpens, views)}%"></div></div><b>${concise(dashboard.telegramOpens)}</b></div>
          <div class="funnelRow"><span>Join Clicks</span><div class="track"><div class="bar orange" style="width:${funnelWidth(dashboard.joinClicks, views)}%"></div></div><b>${concise(dashboard.joinClicks)}</b></div>
          <div class="funnelRow"><span>TG Joins</span><div class="track"><div class="bar green" style="width:${funnelWidth(dashboard.telegramJoins, views)}%"></div></div><b>${concise(dashboard.telegramJoins)}</b></div>
        </div>
      </section>
        ...
        <div class="list">
          ${state.content.length ? state.content.slice(0, 8).map((item) => `
            <article class="item">
              ...
              <div class="mini">
                <div><span>Views</span><b>${concise(item.views)}</b></div>
                <div><span>Unique</span><b>${concise(item.unique)}</b></div>
                <div><span>Clicks</span><b>${concise(item.clicks)}</b></div>
                <div><span>Joins</span><b>${concise(item.joins)}</b></div>
